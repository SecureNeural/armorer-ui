...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: `${agent.name} — Armorer`, description: agent.tagline };
}
...
export default async function AgentDetailPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <Link href="/agents" className="mb-6 inline-flex items-center gap-1 text-sm text-zinc-400 hover:text-white">
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="mb-8 flex items-center gap-4 text-sm text-zinc-500">
            <span className="flex items-center gap-1"><Download className="h-4 w-4" /> {agent.pulls.toLocaleString()} installs</span>
            <span className="flex items-center gap-1"><Star className="h-4 w-4" /> {agent.stars} stars</span>
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {agent.features.map((feature) => (
                <li key={feature} className="flex items-center gap-2 text-sm text-zinc-400">
                  <Box className="h-4 w-4 shrink-0 text-emerald-500" /> {feature}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="mb-8 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
            ...
            {(agent.riskLevel === "high" || agent.riskLevel === "critical") && (
              <div className="mt-4 flex items-start gap-2 rounded-lg border border-amber-800 bg-amber-950/50 p-3 text-sm text-amber-400">
                ...
                This agent has elevated risk. Armorer enforces strict sandboxing, command allow-lists, and real-time monitoring.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5 space-y-3">
            <FavoriteButton itemType="agent" itemSlug={slug} />
            {agent.repoUrl && (
              <a href={agent.repoUrl} target="_blank" rel="noopener noreferrer" className="flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 py-2 text-sm text-zinc-300 transition-colors hover:text-white">
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
              ...
              <div className="space-y-2">
                {compatibleSkills.map((skill) => (
                  <Link key={skill.slug} href={`/skills/${skill.slug}`} className="block rounded-lg border border-zinc-800 p-3 text-sm transition-colors hover:border-zinc-700">
